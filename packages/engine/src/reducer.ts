...
import { InvalidActionError, assertNever } from './errors';
import { 
    applyActivateEffect,
    applyPlayCard,
    applyAttachDon,
...
export function reducer(state: GameState, action: GameAction): GameState {
    ...
    switch (action.type) {
        ...
        case 'CHOOSE_NEXT_EFFECT':
            state = applyChooseNextEffect(state, action);
            break;
        case 'CHOOSE_TARGETS':
            throw new InvalidActionError(`${action.type} is not yet implemented`);
        // Generated by genChooseReorder and accepted by validate, but never applied —
        // it fell through this switch silently until the exhaustiveness check below
        // surfaced it. Unreachable today because nothing sets a REORDER decision point;
        // live the moment LOOK does.
        case 'SUBMIT_REORDER':
            throw new InvalidActionError(`${action.type} is not yet implemented`);
        default:
